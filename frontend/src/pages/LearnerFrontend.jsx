...
// Vite reads every frontend source file's real text at build time - so this page
// always shows the actual current code, never a stale hand-copied version.
const modules = import.meta.glob('/src/**/*.{js,jsx,json}', { query: '?raw', import: 'default', eager: true });
const sourceByPath = Object.fromEntries(
  Object.entries(modules).map(([key, content]) => [`frontend${key}`, content])
);

const frontendSection = fileMap.find((s) => s.section === 'Frontend');

const flatFiles = frontendSection.groups.flatMap((g) =>
...
export default function LearnerFrontend() {
        ...
        <div className="max-w-3xl mx-auto px-4 py-10 sm:py-14">
          ...
          <motion.div initial="hidden" animate="show" variants={container}>
            <motion.div variants={item} className="mb-6">
              <p className="text-xs font-semibold tracking-wide text-sky-400 uppercase mb-2">Frontend</p>
              <h1 className={`text-2xl sm:text-3xl font-bold mb-2 ${isDark ? 'text-white' : 'text-slate-900'}`}>Every frontend file, in full</h1>
              <p className={`text-sm sm:text-base max-w-xl ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>
                All {totalFiles} files under frontend/src. Click a file's header to expand its code - each one
                already has its explanation open below it.
              </p>
            </motion.div>

            <motion.div variants={item} className="mb-8">
              <SortToggle mode={sortMode} setMode={setSortMode} isDark={isDark} accent="bg-sky-500 text-white" />
            </motion.div>

            {sortMode === 'folder' ? (
              frontendSection.groups.map((g) => (
